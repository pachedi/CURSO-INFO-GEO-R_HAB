...
Ahora trabajemos con los 24 Partidos del GBA. Debemos realizar un nuevo filtro que incluya los nombres de estos 40 partidos.
```{r}
...
area = round(set_units(st_area(amba), km^2,2),4)
...
ggplot(data=amba)+
  geom_sf(data = caba, col=NA, fill="gray")+
  geom_sf(aes(fill=area_numero), alpha= 0.7)+
...
ggplot(data=amba)+
  geom_sf(aes(fill=densidad_poblacional), col=NA)+
...
ggsave("quilmes.png", plot = p, width = 10, height = 6, units = "in", dpi = 300,
       bg="white")
...
quilmes <- read.csv("bases_clase2/quilmes/quilmes_censo_link.csv")
...
ggplot() +
  ...
  labs(title = "Barrios populares y elevación del suelo",
       subtitle = "Partido de Quilmes, Pcia de Buenos Aires", fill = "Elevacion (metros)")
